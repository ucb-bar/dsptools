...
trait HasDspIPXact extends HasIPXact {
  ...
  def makeDspBlockInterfaces(mmref: String): BusInterfaces = {
    ...
    val streamOutInterface = makeAXI4StreamInterface("io_out", "data_out", direction = true)
    val axiInterface = makeAXI4Interface(mmref, "io_axi", "axi4_slave", direction = false)
  ...
  def makeXbarInterfaces(inputs: Int, outputs: Int, mmref: String, asref: String): BusInterfaces = {
    val inputInterfaces = (0 until inputs).map(i => makeAXI4Interface(s"${mmref}_$i", s"io_in_$i", s"io_in_$i", direction = false, outputs, "io_out"))
    val outputInterfaces = (0 until outputs).map(i => makeAXI4Interface(s"${asref}_$i", s"io_out_$i", s"io_out_$i", direction = true))
  ...
  def makeDspBlockMemoryMaps(mmref: String, baseAddress: BigInt, registers: mutable.HashMap[String, BigInt]): MemoryMaps = {
  ...
  def makeDspBlockPorts(bits_in: Int, bits_out: Int)(implicit p: Parameters): ModelType.Ports = {
    ...
    val streamInPorts = makeAXI4StreamPorts("io_in", direction = false, bits_in)
    val streamOutPorts = makeAXI4StreamPorts("io_out", direction = true, bits_out)
  ...
  def makeDspBlockComponent(_baseAddress: BigInt, uuid: Int, name: String)(implicit p: Parameters): ComponentType = {
    ...
    val model = makeModel(makeDspBlockPorts(10, 10)) // bits_in, bits_out))
