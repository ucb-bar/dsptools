...
trait DspGeneratorApp extends GeneratorApp {
  ...
  def generateIPXact {
    ...
    busInterfaces.getBusInterface().addAll(toCollection(Seq(makeInputInterface, 
      makeAXIInterface(outMemMapName), makeAXIInterface(memMapName))))
    ...
    //))
    // TODO: get base addresses for these
    val memoryMaps = new MemoryMaps
    memoryMaps.getMemoryMap().add(makeMemoryMap(memMapName, BigInt(0)))
    memoryMaps.getMemoryMap().add(makeMemoryMap(outMemMapName, BigInt(0)))
