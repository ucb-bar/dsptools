...
class ParameterizedSaturatingAdderTester[T <: Data: Integer](c: ParameterizedSaturatingAdder[T], width: Int)
  ...
  for {
    ...
    expect(
      c.io.normalSum,
      overflowint(i + j),
      s"parameterized normal adder $i + $j => $resultNormal should have been ${overflowint(i + j)}"
    )
    expect(
      c.io.saturatedSum,
      saturateint(i + j),
      s"parameterized saturating adder $i + $j => $resultSaturated should have been ${saturateint(i + j)}"
    )
  }
}

class ParameterizedSaturatingAdderSpec extends AnyFlatSpec with ChiselScalatestTester {
  behavior.of("parameterized saturating adder circuit on SInt")

  it should "allow registers to be declared that infer widths" ignore {

    val width = 3
    def getSInt: SInt = SInt(width.W)

    test(new ParameterizedSaturatingAdder(getSInt))
      .runPeekPoke(new ParameterizedSaturatingAdderTester(_, width))
  }
