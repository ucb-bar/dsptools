...
class ConstantTapTransposedStreamingTester(c: ConstantTapTransposedStreamingFIR[SInt, Int]) extends PeekPokeTester(c) {
  val smallest = -5
  val biggest = 5
  ...
  def checkAnswer(n: Int): Int = {
    ...
    c.taps.zipWithIndex.foldLeft(0) {
      case (s, (tap, idx)) =>
...
class TransposedStreamingTester(c: TransposedStreamingFIR[SInt]) extends PeekPokeTester(c) {
...
class TransposedStreamFIRSpec extends AnyFlatSpec with ChiselScalatestTester {
  "ConstantTapTransposedStreamingFIR" should "compute a running average like thing" in {
    val taps = 0 until 3

    test(new ConstantTapTransposedStreamingFIR(SInt(10.W), SInt(16.W), taps))
      .runPeekPoke(new ConstantTapTransposedStreamingTester(_))
  }
}
