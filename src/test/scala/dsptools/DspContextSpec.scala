...
class DspContextSpec extends AnyFreeSpec with ChiselScalatestTester with Matchers {
  ...
  "Test proper nesting of DspContext over module instantiation" in {
    test(new ContextNestingTop(UInt(4.W), UInt(5.W)))
      .runPeekPoke(new ContextNestingTester(_))
  }
}

class ContextNestingTester(c: ContextNestingTop[UInt]) extends PeekPokeTester(c) with PeekPokeDspExtensions {
  poke(c.io.in1, 15.0)
  poke(c.io.in2, 2.0)
