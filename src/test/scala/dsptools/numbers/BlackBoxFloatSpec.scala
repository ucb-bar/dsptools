...
class BlackBoxFloatAdderTester(c: BlackBoxFloatAdder) extends PeekPokeTester(c) with PeekPokeDspExtensions {
  poke(c.io.a, 2.1)
  poke(c.io.b, 3.0)
...
class FloatOpTester[T <: FloatOps](c: T, testTrigFuncs: Boolean = true)
...
class BlackBoxFloatSpec extends AnyFlatSpec with ChiselScalatestTester {
  "A BlackBoxed FP block" should "work" in {
    test(new BlackBoxFloatTester)
      .withAnnotations(Seq(VerilatorBackendAnnotation))
      .runUntilStop()
  }

  "basic addition" should "work with reals through black boxes" in {
    test(new BlackBoxFloatAdder)
      .withAnnotations(Seq(VerilatorBackendAnnotation))
      .runPeekPoke(new BlackBoxFloatAdderTester(_))
  }

  "float ops" should "work with trig functions" in {
    test(new FloatOpsWithTrig)
      .withAnnotations(Seq(VerilatorBackendAnnotation))
      .runPeekPoke(new FloatOpTester(_))
  }

  "float ops" should "work without trig functions" in {
    test(new FloatOpsWithoutTrig)
      .withAnnotations(Seq(VerilatorBackendAnnotation))
      .runPeekPoke(new FloatOpTester(_, testTrigFuncs = false))
  }

  "greater than" should "work with negatives" in {
    test(new NegCircuit)
      .withAnnotations(Seq(VerilatorBackendAnnotation))
      .runPeekPoke(new NegCircuitTester(_))
  }
}
...
class NegCircuitTester(c: NegCircuit) extends PeekPokeTester(c) with PeekPokeDspExtensions {
  poke(c.io.in1, -1.0)
  poke(c.io.in2, -2.0)
